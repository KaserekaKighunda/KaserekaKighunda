# 👋 Salut, moi c’est **Kasereka Kighunda**

💻 **Développeur Full Stack** passionné par la technologie, l’innovation et le développement de solutions intelligentes.  
Je conçois et construis des applications modernes, performantes et intuitives.

---

### 🚀 À propos de moi

- 🔭 Je travaille actuellement sur plusieurs **projets personnels et collaboratifs**.  
- 🌱 J’explore en ce moment le **Machine Learning avec Python** et je renforce mes compétences en **Dart / Flutter**.  
- 💡 J’aime apprendre de nouvelles technologies et relever des défis techniques.  
- 🤝 Ouvert aux **opportunités de collaboration** sur des projets open-source ou professionnels.  

---

### ⚙️ Technologies & Outils

| Langage / Outil | Description |
|------------------|-------------|
| 🐍 **Python** | Machine Learning, Data Science |
| 💙 **Dart / Flutter** | Développement d’applications mobiles multiplateformes |
| 🖥️ **VB.NET** | Développement d’applications de bureau |
| 🗄️ **MySQL** | Gestion et optimisation de bases de données |
| 🧠 **Git & GitHub** | Gestion de versions et collaboration |

---

### 📊 Statistiques GitHub

![Statistiques GitHub de Kasereka Kighunda](https://github-readme-stats.vercel.app/api?username=KaserekaKighunda&show_icons=true&theme=tokyonight)

![Langages les plus utilisés](https://github-readme-stats.vercel.app/api/top-langs/?username=KaserekaKighunda&layout=compact&theme=tokyonight)

---

### 🌐 Me contacter

- 🔗 **LinkedIn** : [Kasereka Kighunda](https://www.linkedin.com/in/kasereka-kighunda-52312022a/)  
- 🧑‍💻 **GitHub** : [KaserekaKighunda](https://github.com/KaserekaKighunda)

---

⭐ N’hésite pas à suivre mon profil pour découvrir mes futurs projets !

> _“La technologie n’est pas qu’un outil, c’est une façon de penser le monde différemment.”_
